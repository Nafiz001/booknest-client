...
const MapCore = ({ compact = false }) => (
  <div className={`relative overflow-hidden rounded-xl border border-primary/20 ${compact ? 'h-64' : 'h-96'} shadow-lg`}>
    <MapContainer
    ...
    >
      ...
      />
      {cities.map((city) => (
        <Marker key={`${city.name}-${city.state}`} position={[city.lat, city.lng]} icon={blueIcon}>
          <Popup>
